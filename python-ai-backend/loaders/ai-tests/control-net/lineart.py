...
app = Quart(__name__)
checkpoint = "lllyasviel/control_v11p_sd15s2_lineart"
...
async def generate_image(prompt):
    try:
        with Image.open('./snapshots/snapshot_0.png').convert("RGB").resize((512, 768)) as image:
            ...
            generated_image = pipe(prompt, image, generator=generator, negative_prompt='cropped, lowres, poorly drawn face, out of frame, poorly drawn hands, too many fingers, blurry, bad art, blurred, text, watermark, disfigured, deformed, closed eyes').images[0]
            torch.cuda.empty_cache()  # Clear CUDA cache to release GPU memory
            generated_image.save(f'./gen_pics/newpic.png')
